#!/usr/bin/env node
/**
 * Close all open cases stuck at Bill Submission (Bulk admin action).
 * Usage: node scripts/close-bill-submission-cases.mjs [--dry-run]
 */
...
const SKIP_NOTE = 'Skipped — Bill Submission disabled.';
const CLOSE_NOTE = 'Bulk closed — Bill Submission stage disabled.';
...
function closeStages(stages) {
  ...
  return (stages ?? []).map((s) => {
    ...
    if (name === 'Bill Submission' && s.status !== 'Approved') {
      return {
        ...s,
        stage: name,
        status: 'Approved',
        approvedAt: s.approvedAt ?? now,
        adminNotes: s.adminNotes || SKIP_NOTE,
      };
    }
...
const { data: cases, error } = await sb
  ...
  .in('current_stage', ['Bill Submission', 'Collection'])
...
if (!cases?.length) {
  console.log('No open cases at Bill Submission.');
  process.exit(0);
}
