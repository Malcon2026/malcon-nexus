...
The connected Vercel project (e.g. `malcon-nexus-gamma.vercel.app`) deploys **`npm run build:vercel-legacy`** only — a beta thank-you notice (clears old service workers), not the app. Real builds use `npm run build` + `npm run deploy:hostinger`.
