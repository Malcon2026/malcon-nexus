...
export const EmployeeFoodSection: React.FC = () => {
  ...
  const toggleMeal = (meal: FoodMeal) => {
  ...
  const handleSubmit = async () => {
    if (locked || submitting) return;
    setError(null);
    if (!draft.breakfast && !draft.lunch && !draft.dinner) {
      setError('Select at least one meal (Breakfast, Lunch, or Dinner), then tap Submit.');
      return;
    }
    setSubmitting(true);
    try {
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardBody className="space-y-4">
          ...
          {!locked && (
            <p className="text-xs text-gray-500">Tap the meals you need today, then Submit once.</p>
          )}
