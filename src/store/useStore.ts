...
interface AppState {
  ...
  createCase: (
    caseData: Partial<ImplantCase> & {
      ...
      /** Return category at create — auto-advance when case reaches pickup. */
      pickupReturnOutcome?: import('../types').ReturnOutcome;
    },
  ) => Promise<void>;
...
const locationTripDbError = (err: unknown, fallback: string): string => {
  ...
  if (message.includes('idx_location_trips_one_open')) {
    return 'A trip is already in progress. Press Reached first.';
  }
  if (message.includes('row-level security') || message.includes('RLS')) {
    return 'Could not save. Please log out and log in again, then retry.';
  }
...
const persistLocationTrip = async (trip: LocationTrip): Promise<{ error: string | null }> => {
  if (USE_SUPABASE) {
    try {
      await sbLocationTripRepo.insert(trip);
    } catch (err) {
      console.error('[location-trip] persist failed:', err);
      return { error: locationTripDbError(err, 'Failed to start location trip') };
    }
    const list = Database.getAll<LocationTrip>('locationTrips');
    setCache('locationTrips', [trip, ...list.filter((t) => t.id !== trip.id)]);
    return { error: null };
  }
  const list = Database.getAll<LocationTrip>('locationTrips');
...
const updateLocationTrip = async (
  id: string,
  updates: Partial<LocationTrip>,
): Promise<{ error: string | null }> => {
  if (USE_SUPABASE) {
    try {
      await sbLocationTripRepo.update(id, updates);
    } catch (err) {
      console.error('[location-trip] update failed:', err);
      return { error: locationTripDbError(err, 'Failed to complete location trip') };
    }
    const list = Database.getAll<LocationTrip>('locationTrips');
    setCache(
      'locationTrips',
...
function buildAutoAdvanceFromSubmit(
  ...
  const frozenPostpone = isPostponedCase(c);

  if (!frozenPostpone && skipSelfSurgery) {
  ...
  if (!frozenPostpone && skipPickupForUsedNoReturn) {
  ...
  if (!frozenPostpone && completeParkedFromPickup) {
  ...
  const pointer = frozenPostpone
    ? postponedCasePointer(c)
    : computeOpenCasePointer({ stages: updatedStages, cancelReason: c.cancelReason });
  ...
  const advanceLog = {
    ...
    details: advancingToFcfs
      ...
      : completeParkedFromPickup
        ...
        : frozenPostpone
          ? `${submittedName} submitted — case postponed; stays at ${pointer.currentStage}. ${opts.autoNotes}`
  ...
  return {
    ...
    advanceLog,
    nextStage: frozenPostpone || completeParkedFromPickup ? null : next,
  };
}
...
const applyUserSession = (
  ...
  if (user.role !== 'admin' && ADMIN_ONLY_TABS.includes(activeTab)) {
    activeTab = 'dashboard';
  }
...
export const useStore = create<AppState>((set, get) => ({
  ...
  createCase: async (caseData) => {
    ...
    const rawStages = WORKFLOW_STAGES.map((stage) => {
      ...
      const pickupReturnOutcome = caseData.pickupReturnOutcome;
      const isSpecialReturn =
        stage === 'Pickup from Hospital' && Boolean(pickupReturnOutcome);
      const emp =
        isFcfsStage(stage) || isSelfSurgery || isSpecialReturn
          ? null
          : (assignments[stage as keyof typeof assignments] ?? null);
      const assistantEmp =
      ...
      return {
        ...
        ...(isSpecialReturn && !isSkipped ? { returnOutcome: pickupReturnOutcome } : {}),
      };
    });
  ...
  approveStage: async (caseId, adminNotes) => {
    ...
    if (completeParkedFromPickup && !isPostponedCase(c)) {
    ...
    let updatedStages = applyApprovalWithSelfSurgerySkip(c.stages, currentName, {
      approvedAt,
      adminNotes,
      skipSelfSurgery: isPostponedCase(c) ? false : skipSelfSurgery,
    });
    if (!isPostponedCase(c) && skipPickupForUsedNoReturn) {
      updatedStages = applyApprovalWithPickupUsedNoReturnSkip(updatedStages, currentName, {
        approvedAt,
        adminNotes,
        skipPickup: true,
    ...
    if (!isPostponedCase(c) && advancingToFcfs && next) {
      updatedStages = normalizeCaseStages(
        updatedStages.map((s) => {
    ...
    const holdPostponed = isPostponedCase(c);
    const newLog = {
      id: `log-${Date.now()}`,
      caseId,
      action: advancingUnassigned && !holdPostponed ? `Stage Advanced: ${c.currentStage}` : `Stage Approved: ${c.currentStage}`,
      performedBy: state.currentUser.name,
      performedByRole: 'admin' as const,
      timestamp: approvedAt,
      details: advancingToFcfs
        ...
        : advancingUnassigned
          ? `Admin moved ${c.currentStage} forward with no assignee. Next: ${next}.${skipSelfSurgery ? ' Self surgery skipped.' : ''}${skipPickupForUsedNoReturn ? ' Return skipped (used/no return).' : ''} ${adminNotes}`
      ...
      status: holdPostponed
        ...
        : advancingUnassigned
          ? advancingToFcfs
            ...
            : 'Draft'
          : 'Approved',
      ...(advancingUnassigned && next && !holdPostponed
        ? {
            currentStage: next,
    ...
    if (advancingUnassigned && !holdPostponed && c.assignedEmployee && canUpdateEmployeeStats) {
  ...
  forceAdvanceCase: async (caseId, targetStage, adminNotes) => {
    if (!FORCE_ADVANCE_ENABLED) {
      throw new Error('Force advance is temporarily disabled.');
    }
    const state = get();
      ...
      c.stages.map((s, i) => {
        if (i >= currentIdx) {
          return {
            ...
            status: 'Approved' as const,
            approvedAt: s.approvedAt ?? now,
            adminNotes: i === currentIdx ? adminNotes : skipNote,
          };
        }
        return s;
      }),
  ...
  markSurgerySelfPerformed: async (caseId, notes) => {
    ...
    if (isPostponedCase(c)) {
      return { error: 'Cannot advance self surgery while the case is postponed. Clear postpone or wait for the new surgery date.' };
    }
  ...
  confirmPostSurgeryTeam: async (caseId, employeeIds) => {
    ...
    let returnOutcomeSet: import('../types').ReturnOutcome | null = null;
    ...
    for (const kind of CASE_DUTY_KINDS) {
      ...
      if (kind === 'return' && isReturnDutySpecialValue(empId)) {
        const outcome = returnDutyIdToOutcome(empId);
        if (!outcome) continue;
        const pickupRec = findStageRecord(stages, 'Pickup from Hospital');
        if (pickupRec?.returnOutcome === outcome && !pickupRec.assignedEmployee) continue;

        returnOutcomeSet = outcome;
        duties.return = { assignedEmployee: null, assignedAt: null };
        stages = stages.map((s) =>
          normalizeWorkflowStageName(s.stage) === workflowStage
            ? {
                ...
                assignedEmployee: null,
                assignedAt: null,
                status: s.status === 'Assigned' ? ('Pending' as const) : s.status,
              }
            : s,
        );
        ...
        const log = {
          ...
          caseId,
          action: `${label}: ${returnOutcomeLabel(outcome)}`,
          performedBy: state.currentUser.name,
        ...
        newLogs.push(log);
        persistActivity(
          createActivityEvent(log.action, 'case', caseId, c.caseNumber, state.currentUser.name, performerRole, log.details),
        );
        continue;
      }
      ...
      stages = stages.map((s) =>
        normalizeWorkflowStageName(s.stage) === workflowStage
          ? {
              ...
              ...(kind === 'return' ? { returnOutcome: undefined } : {}),
            }
          : s,
      );
    ...
    const after = get().cases.find((x) => x.id === caseId);
    if (
      returnOutcomeSet &&
      after &&
      !isPostponedCase(after) &&
      normalizeWorkflowStageName(after.currentStage) === 'Pickup from Hospital' &&
      after.status !== 'Waiting For Approval'
    ) {
      const pickupRec = findStageRecord(after.stages, 'Pickup from Hospital');
      if (pickupRec?.status !== 'Approved') {
        await get().approveStage(
          caseId,
          `${returnOutcomeLabel(returnOutcomeSet)} — auto-advanced from return assignment.`,
        );
      }
    }
  },
  ...
  repairStuckPickupReturnOutcomes: async () => {
    ...
    for (const c of state.cases) {
      ...
      try {
        await get().approveStage(
          c.id,
          `${returnOutcomeLabel(pickupRec.returnOutcome)} — auto-advanced from return category.`,
        );
      } catch (err) {
  ...
  submitStage: async (caseId, notes, photos, onUploadProgress, restockOutcome, returnOutcome) => {
    ...
    try {
      ...
      const pointer = isPostponedCase(c)
        ? postponedCasePointer(c)
        : computeOpenCasePointer({ stages: updatedStages, cancelReason: c.cancelReason });
      ...
      const activity = createActivityEvent(
        ...
        'employee',
        isPostponedCase(c)
          ? `${submitStageName} submitted — case postponed; stays at ${c.currentStage}.`
  ...
  startLocationTrip: async (position, from = null, to = null) => {
    ...
    const persistResult = await persistLocationTrip(trip);
  ...
  completeLocationTrip: async (position) => {
    ...
    const persistResult = await updateLocationTrip(open.id, updates);
    if (persistResult.error) return persistResult;
