-- Parallel workflow: employees assigned on Pickup / Clean / Restock must read & submit
-- even when case header (assigned_employee_id / current_stage) points at another stage.
--
-- Run in Supabase Dashboard → SQL Editor after fix-cases-employee-submit-rpc.sql.
    ...
    AND EXISTS (
      ...
      WHERE elem->'assignedEmployee'->>'id' = emp_id::text
        AND COALESCE(elem->>'status', 'Pending') IN (
          'Pending', 'Assigned', 'In Progress', 'Changes Requested', 'Rejected'
        )
    );
$$;
...
CREATE POLICY "cases_employee_assigned" ON cases FOR SELECT USING (
  ...
  OR employee_is_assignee_on_open_stage(stages, current_employee_id())
  OR EXISTS (
    SELECT 1
...
BEGIN
  ...
  IF role <> 'admin' THEN
    ...
    IF existing.assigned_employee_id IS DISTINCT FROM emp_id
       AND NOT employee_is_assignee_on_open_stage(existing.stages, emp_id) THEN
